...
// RCS       :
//
// $Source: /home/franco/CVS/home/C653_Final_09.8150/Project/CheckForLiving.java,v $
// $Revision: 1.1 $
...
class CheckForLiving extends Thread implements RecurredEvent {
	...
	public void run() {
		...
		for (Enumeration e = playerDB.getPlayers() ; e.hasMoreElements() ; )  {
			...
			try {
				aliveConnection = new AliveConnection(curPlayer.getInetAddress(),
																  curPlayer.getPort(),
																  playerDB, curPlayer);
			...
			if (aliveConnection != null) {
				...
				try { aliveConnection.join(); } catch(InterruptedException ie) { }
			}
			...
			System.out.println("CHECK_FOR_LIVING:     " +
									 ...
									 " has been alive for " + 
				((double)curPlayer.getSecondsAliveSinceLastResourceAllocation()/60.0)
